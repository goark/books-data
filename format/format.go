...
func ByTemplateFile(obj interface{}, tmpltPath string) (*bytes.Buffer, error) {
	...
	if err != nil {
		return nil, errs.Wrap(err, "error in format.ByTemplateFile() function")
	}
	defer file.Close()
...
func Do(obj interface{}, tmplt string) (*bytes.Buffer, error) {
	...
	if err != nil {
		return nil, errs.Wrap(err, "error in format.Do() function")
	}
	buf := &bytes.Buffer{}
	if err := t.Execute(buf, obj); err != nil {
		return buf, errs.Wrap(err, "error in format.Do() function")
	}
	return buf, nil
}
